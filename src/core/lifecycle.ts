...
const getOwner = (name: LifecycleHooks) => {
  assert(Owner, `the ${name} lifecycle called outside "setup()" will never be run.`);
  return Owner;
};

export const onMounted = (fn: () => void) => getOwner(LifecycleHooks.MOUNTED).onMounted.push(fn);
